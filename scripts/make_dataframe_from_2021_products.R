...
products <- list.files("inputs/2021/products", full.names = TRUE)

names(products) <- list.files("inputs/2021/products") %>% 
...
dataset <- 
  dataset %>% 
  distinct(name, description, flowerydescription, availability, sku, price, .keep_all = TRUE)

write_csv(dataset, "outputs/data/2021_dataset.csv")
