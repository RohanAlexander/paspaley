---
title: "One weird trick to get 33% off Paspaley"
author: "Rohan"
date: "01/01/2019"
...
# Overview
I scraped the Paspaley website on Boxing Day 2017 and again on Boxing Day 2018. There's a bunch of interesting aspects going on, but the one that stands out the most is the 33% price reduction in the XX.


# Introduction
Paspaly make the best pearl jewelry in the world. Unfortunately it's priced accordingly so actual Paspaley purchases are somewhat rare for me, but isn't half the fun of luxury products comtemplating them? That said, there's only so much time that I could spend looking at photos of pearls before I began to wonder about their pricing strategy, so I scraped the website on Boxing Day 2017, and again on Boxing Day 2018. After cleaning the data we can have a bit of a look at what's going on.

The nice aspect of this project is that it's a small-scale project that showcases a variety of data skills that I use in my academic life, but that rarely make it to the final paper such as: acquisition; cleaning; analysis; and communication. In this write-up I'm going to explain each step so that you can do you own analysis of a similar type. If you'd just like to see the project folder then it's available on my GitHub page: https://github.com/RohanAlexander/paspaley. Note that I haven't included the scraped html files there but if there's something in the html that you think I haven't taken advantage of then let me know and I can update the scripts.


# Data acquisition
There was an update to the Paspaly website at some point in the year where they moved away from Shopify. So I'll just focus on the most recent version of the website here. The first step when acquiring data from a website is to check for an API. In this case, it doesn't look like they have one. The next step is to look at the relevant robot.txt file: https://www.paspaley.com/robots.txt. In this case it doesn't give us much information, but it helpfully provides a link to the XML sitemap: https://www.paspaley.com/pub/sitemap.xml.

```{r, echo=FALSE}
knitr::include_graphics("images/sitemap.png")
```

That appears to give us a nice list of the products on the website and their address so that we can get their details.

We can use the R package rvest to quickly grab the relevant information from this file. 

**ADD R SCRIPT.**

Although the sitemap should be giving us everything, it's worth seeing if we can follow the links to get any other information that's missing.

Looking at the website, Paspaley organises the products by category (e.g. bracelet, earrings, etc) and also by collection (e.g. Monsoon, Rockpool, etc). Save each of those pages and we can then scrape the links for each of the individual products from them using 
...
Now that we have the list of the products that we'd like details about, we need to get the the page for each product. 
...
Finally, in anything that is written there are always typos and mistakes that fresh eyes notice, but that the author could read a thousand times and miss. So we'd like to fix a bit of the website content, so that they don't continue into our dataset (instead we can make new mistakes!).
...
In general, it's clear that Paspaley has changed how they are using their website. It's gone from acting as a place of commerce (e.g. being able to buy gift vouchers on it) to being something that feels more about allowing people to engage with the brand in a less financial way, perhaps as a complement to their Instagram account, but you still go into the stores when you want to buy.

Based on the SKUs, there are 77 products that are on the 2017 and 2018 versions of the website. 
...
Even though Paspaley has a lot of products at that it doesn't sell through the website, the prices that are shown on the website are still generally fairly high!

```{r, include = TRUE}
library(scales) # Just used to get away from scientific notation in the last line.
...
There is a large skew - in 2018 there were five products with a price over \$100,000; down from seven in 2017. Interestingly if we focus on products that have a price less than \$50,000 we again see this skew, although at this less expensive (still way too expensive for me) level it's being driven by the 'other' collection These tends to be products such as clasps or chains.
...
```{r, include = TRUE}
ggplot(data = the_data, aes(category, log(price))) +
  geom_boxplot() +
  facet_wrap(vars(year), nrow = 2) +
  theme_classic() 
```
...
Price comparisons by collection and by category mean.

Price comparisons by year for those that were in both.



# Weaknesses
An obvious weakness of the analysis is that it's just based on what I could find on the website. With a better dataset we could do more interesting analysis and be more confident in our results.
